...
#define device1_E_pin   4   //User must replace your_E_pin with their pin number (compile error will occur if no number defined)
#define device1_D_pin   5   //User must replace your_D_pin with their pin number (compile error will occur if no number defined)
...
void setup() {
  Serial.begin(115200);
  while (!Serial);  //w`ait for serial port to connect
